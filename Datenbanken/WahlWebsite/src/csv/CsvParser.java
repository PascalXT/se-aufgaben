...
public class CsvParser {
  ...
  private String getBundesland(int wahlkreisID) throws SQLException {
    String sql = "SELECT " + DB.kBundeslandName + " FROM " + datenbank.bundesland() + " WHERE "
        + DB.kID + "=" + "(SELECT " + DB.kForeignKeyBundeslandID + " FROM "
        + datenbank.wahlkreis() + " WHERE " + DB.kID + "=" + wahlkreisID + ")";
    ResultSet resultSet = datenbank.executeSQL(sql);
    resultSet.next();
